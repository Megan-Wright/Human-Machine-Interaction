...
The heaters we use indoors use a negative feedback system. The heater heats up the room and uses the thermostat (sensor) to check the current temperature. If the room temperature is lower than the set temperature, then it keeps the heater on, making the room warmer. If it’s the room temperature is the same or higher then it turns the heater off. This is so that it can maintain or cool the room.
\end{mdframed}
\vspace*{\baselineskip}
...
\begin{mdframed}
When the sampling rate is decreased, the period of oscillation of the robot around the target point increases. The robot also travels further awa from the target before correcting its position, as there is a longer time between sampling and so it takes longer to sense that it is not in the correct place.
\end{mdframed}
\vspace*{\baselineskip}
...
\begin{mdframed}
\begin{center}
	\includegraphics[width=\textwidth]{20190314_160457.jpg}
\end{center}
\begin{center}
	\includegraphics[width=\textwidth]{20190314_160441.jpg}
\end{center}
We decided to put the sensors to face the wall $45^{\circ}$. We thought about placing the sensor horizontally to the wall. However, because it gets the value from the wall right next to the robot, it would crash into the wall when the sharp curve is upcoming, as it can't predict the distance from the wall ahead. By placing the sensor in $45^{\circ}$, the robot was able to check the distance from the upcoming wall, predicting whether it is curved or not. Other than that, we kept our robot's design simple, trying to use the minimum amount of the blocks. We felt if the sensor is connected to more complex structure, it is easier to get shaken, resulting in unstable reading.
\end{mdframed}
\vspace*{\baselineskip}
...
\begin{mdframed}
We didn't have enough time to run the robot in the maze and when we could, there were too many robots in the arena, so the sensor values were different every time we ran the code. There were some values which could finish the maze without touching the wall but we weren't able to check several times and implement the values we got. If we had more time, we want to check the sensor values thoroughly and set more precise sensor value range or PID values for the robot. Also, we want to try differnt PID tuning methods. As told in the instruction, we used Ziegler-Nichols method when calculating the PID values. However, it would have been better if we could have experimented with another methods, such as Cohen-Coon parameters or Relay (Astrom-Hagglund) method to check which gives the best result for the maze running.
\end{mdframed}
\vspace*{\baselineskip}
...
\begin{center}
	\begin{tabular}{ |c|c|c|c|c| } \hline
		...
		end reached Yes/ metres & 01:56 & 2 & 2 \\ \hline
	\end{tabular}
\end{center}
\end{mdframed}
\vspace*{\baselineskip}
...
\begin{center}
	\begin{tabular}{ | c | p{8cm} | c | } \hline
		...
		Megan Wright & report, experiment & 33.3 \\ \hline
		Rafael Cavagnoli & maze, experiment & 33.3 \\ \hline
	    	Sohyun Park  & report, experiment & 33.3 \\ \hline
	\end{tabular}
\end{center}
