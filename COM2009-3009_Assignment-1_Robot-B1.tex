...
The heater we use indoor uses negative feedback system. The heater heats up the room and uses the thermostat (sensor) to check the current temperature, and if the room temperature is lower than the set temperature, then it keeps the heater on, making the room warmer. If it’s the room temperature is the same or higher then it turns the heater off. so that it can maintain or cool the room.
\end{mdframed}
\vspace*{\baselineskip}
